...
We will talk about some of the important aspects that arise from the special
situation of working in a lazy language.

As an example we will show how to implement a parallel map function in Haskell.

Controlling evaluation
======================

Haskell programmers face a unique challenge when writing parallel programs.
Haskell's laziness makes the order of evaluation unpredictable. To get the
optimal performance out of a parallel Haskell program, the programmer must take
care to explicitly control the order of evaluation. Let's say you want to
compute two expressions `a` and `b` in parallel and suppose that both
expressions depend on the result of some common expression `e`, then naïvely
starting a computation of `a` in parallel with `b` could lead to extra work
being performed. Similarly `b` might depend on `a` in which case the evaluation
of `b` won't be able to finish until `a` is available. This will also result in
more work being performed.

Controlling evaluation with `seq`
---------------------------------

The two previous examples illustrate the need for a way of controlling 1)
forcing evaluations and 2) controlling evaluation order. Partially evaluated
expressions are represented as *thunks* in Haskell. These can be thought of as a
description of how to arrive at a final result. Thunks can be inspected in
`ghci` using `:sprint`. A simple example will show this:

< § let x = "Hi there."
< § :sprint x
< > x = _

The `_`-part here is the unevaluated part of the expression. We see that nothing
is evaluated immediately after the definition of `x`. We can force a value to be
evaluated to *Weak Head Normal Form* (WHNF for short) by using `seq`:
...
`x` to WHNF will only happen once the result of `seq x ()` is needed. So in this
example `x` is not reduced:
...
`fromInteger x`. So really `x` is in WHNF, but it's a lambda-abstraction! Here
is another interesting example of a function behaving unexpectedly:
...
We see that when we first try to `sprint` `s` that no reading has actually taken
place. When we force it to WHNF it suddenly contains not only the first element
of the list, but a decent chunk of null-characters.
...
evaluation order. We get this from `pseq`, but evaluating something to WHNF
cannot always be a sufficiently large task for it to be worthwhile doing in
parallel. This is because there is an overhead associated with starting parallel
computations in Haskell. For that reason we want to be able to explicitly fully
evaluate expressions. The `deepseq` package defines a typeclass `NFData` that
...
this in our program. If we have two expressions `a` and `b` that we wist to
evaluate in parallel, then we can inform GHC like so:
...
to do a reasonable amount of work so that the parallelism outweighs the overhead
of using sparks.

Parmap
======

To illustrate some of the challenges that face the parallel Haskell programmer
we will now implement a parallel map function. All benchmark results given are
times measured with Criterion. A full bench-suite and results are given at the
end.

For testing purposes we define a function `nfib` which serves as a convenient
way of making a bunch of busy-work for your machine.
...
used immediately afterwards in the call to `(:)` so in reality we're not gaining
any parallelism, but suffering the overhead of creating sparks as can be seen
from the benchmark output.

We fix this in our next attempt; what we really want to do is create a spark for
the application of `f` to `x` *as well as* a spark for the rest of the
...
Which runs at 491.9 ms. A great improvement! But there's still an issue; `f x`
is only reduced to WHNF! This was not a problem in our previous computation
because the weak head normal form was actually the same as the normal form. But
what if we called some devious function:
...
As a last remark we want to illustrate that there can be cases where the task
the programmer is mapping is too small for comfort. In this, our 5th task, we're
calling `nfib 5` a given number of times. On my machine `nfib 5` takes approx.
0.05 s.
...
our `parMap` slightly. We need a way to control granularity. We can do that like
so:
